...
import { useFormik, FormikValues, FormikState } from 'formik'

import Button from '../utils/button/Button'
import Text, { TextWeight, TextSize, TextTransform } from '../utils/text/Text'
import Modal from '../utils/modal/Modal'
import { nextId, validationSchema, getOnSubmitFormikAction } from '../../../core/utils/helper'
import { Movie } from '../../../core/interfaces'
import { Genres } from '../../../core/constants'

import styles from './ModalStyles.css'

export const getFormkValidationErrorHendler = (formik: FormikState<FormikValues>) => (name: string): ReactNode | null =>
  formik.touched[name] && formik.errors[name]
    ? (<div className={styles.InputErrors}>
      <Text weight={TextWeight.light} size={TextSize.xs} transform={TextTransform.none} red>
        {formik.errors[name]}
      </Text>
    </div>)
    : null
...
type OwnProps = {
  onAddSubmit: (movieData) => void
}

const AddModal: React.FC<OwnProps> = ({
  onAddSubmit
}) => {
  const getNextId = useMemo(nextId, [nextId])
  const id = getNextId
  ...
  const formik = useFormik({
    initialValues,
    validationSchema,
    onSubmit: getOnSubmitFormikAction(onAddSubmit)
  })
    ...
    <Modal title={'add movie'}>
      <form onSubmit={formik.handleSubmit}>
        ...
        <Text
          weight={TextWeight.bold} red classes={classnames(styles.ModalSubtitle)}
        > genre </Text>
        <div /* className={styles['ModalInput--selectWrapper']} - style for an arrow in select input type */>
          <select
            multiple
            {...formik.getFieldProps('genres')}
            className={styles['ModalInput--select']}
          >
            <option disabled value='Select'>Select Genre (multiple)</option>
        ...
        <div className={styles.ModalActoinBtns} >
          <Button red type='submit'>SUBMIT</Button>
